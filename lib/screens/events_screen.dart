...
class _EventsScreenState extends State<EventsScreen> {
  ...
  /// The slides for the top banner.
  ///
  /// The listings an admin flagged as their sections' heroes — the app's
  /// featured listings — so they lead the screen rather than sitting in a
  /// rail. The mock banners stand in until the feed is in, and whenever
  /// nothing is flagged, so the screen never opens on a blank space.
  List<EventModel> get _banners {
    final feed = DiscoveryFeedState.events;
    if (!feed.isLoaded || feed.heroes.isEmpty) {
      return DummyData.eventsScreenBanners;
    }
    return feed.heroes;
  }
                ...
                ColoredBox(
                  ...
                  child: Column(
                    children: [
                      ...
                      // Tall banner. The black fill and gold side-glow belong
                      // to each card, so they travel with it as the carousel
                      // scrolls — a plate behind the carousel would sit still
                      // while the banners slid across it. The fill also backs
                      // the image, so the glow can't bleed through the
